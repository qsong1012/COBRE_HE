...
class HybridModel(nn.Module):
    def __init__(
        ...
        if isinstance(head, str):
            assert(head == 'avg')
            self.head = MLP(in_features=self.feature_dim, out_features=outcome_dim, outcome_type=outcome_type)
...
class HybridFitter:
    ...
    def extract_features(self, df_pred):
        self.prepare_datasets(
            df_pred,
            'predict',
            batch_size=self.args.batch_size)
        self.get_datasets(
            self.meta_df['predict'],
            'predict')

        self.model.eval()

        for i, sample in tqdm(
                enumerate(self.dataloaders['predict']), total=len(self.dataloaders['predict'])):
            batch_data = sample[0]
            pred_imgs, pred_ids, pred_targets = list(
                map(lambda x: batch_data[x], ['images', 'ids', 'targets']))

            nbatches = pred_imgs.size(0) // self.args.num_patches
            pred_imgs = reshape_img_batch(pred_imgs, self.args.crop_size)

            # forward and backprop
            with torch.set_grad_enabled(True):
                pred_features = self.model(pred_imgs)
                pred_features = pred_features.view(pred_imgs.size(0), 512, -1).permute(0, 2, 1)
                pred_features = pred_features.contiguous().view(nbatches, -1, 512)
                torch.save(pred_features, os.path.join(self.args.features_path, '%08d.pt' % i))

    def predict(self):
        save_path = "predictions/%s/%s" % (self.args.timestr, self.args.random_seed_torch)
        ...
        for i, sample in enumerate(tqdm(self.dataloaders['val'])):
            ...
            with torch.set_grad_enabled(False):
                ...
                torch.save(pred_features.data.cpu(), os.path.join(
                    save_path, 'pred_features_%05d.pt' % i))
                torch.save(pred_preds_p.data.cpu(), os.path.join(
                    save_path, 'pred_preds_p_%05d.pt' % i))
                torch.save(pred_w_raw.data.cpu(), os.path.join(save_path, 'pred_w_%05d.pt' % i))
